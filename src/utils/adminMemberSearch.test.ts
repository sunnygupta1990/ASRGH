...
const member = (overrides: Partial<Member>): Member => ({
  ...
  display_name: 'Sh. Tushal Singhal', category: 'Life Member', designation: 'Community Member',
...
const members = [
  member({}),
  member({ id: '2', member_code: 'MEM-0099', first_name: 'Asha', last_name: 'Gupta', display_name: 'Asha Gupta', category: 'Patron', designation: 'Secretary', current_management: true, phone: '9876543210' }),
];
...
test('member-code and designation searches still work', () => {
  assert.deepEqual(search('MEM-0099').map((item) => item.id), ['2']);
  assert.deepEqual(search('secretary').map((item) => item.id), ['2']);
});

test('category and management filters still compose with search', () => {
  assert.deepEqual(filterAdminMembers(members, { search: 'asha', category: 'Patron', management: 'yes' }).map((item) => item.id), ['2']);
