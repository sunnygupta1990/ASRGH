...
export const AdminSocialWork: React.FC<{ onNavigateTab: (tab: string) => void }> = ({ onNavigateTab }) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (selectedActivity) {
      updateSocialWorkActivity(selectedActivity.id, formData);
    } else {
      ...
      addSocialWorkActivity(newAct);
    }
    setIsEditing(false);
  };

  const handleAddPhotoToAct = () => {
    if (!photoManagingAct || !newPhotoUrl.trim()) return;
    const updatedPhotos = [...(photoManagingAct.photos || []), newPhotoUrl.trim()];
    updateSocialWorkActivity(photoManagingAct.id, { photos: updatedPhotos });
    setPhotoManagingAct({ ...photoManagingAct, photos: updatedPhotos });
    setNewPhotoUrl('');
  };

  const handleRemovePhotoFromAct = (photoIndex: number) => {
                        ...
                        <button
                          onClick={() => {
                            if (confirm(`Are you sure you want to delete "${act.title}"?`)) {
                              deleteSocialWorkActivity(act.id);
                            }
                          }}
